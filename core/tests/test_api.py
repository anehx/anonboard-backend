...
class TopicAPITests(JSONAPITestCase):
    ...
    def test_get_topic(self):
        ...
        self.assertEqual(result['data']['attributes']['name'],        self.topics[0].name)
        self.assertEqual(result['data']['attributes']['identifier'],  self.topics[0].identifier)
        self.assertEqual(result['data']['attributes']['description'], self.topics[0].description)
...
class ThreadAPITests(JSONAPITestCase):
    ...
    def test_get_thread(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(result['data']['attributes']['title'],   self.threads[0].title)
        self.assertEqual(result['data']['attributes']['content'], self.threads[0].content)
